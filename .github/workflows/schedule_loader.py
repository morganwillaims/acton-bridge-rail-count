...
def readable_location_name(tiploc_lookup: dict[str, str], tiploc: str | None) -> str | None:
    if not tiploc:
        return None
    value = tiploc_lookup.get(tiploc)
...
def extract_tiploc(record: dict) -> tuple[str, dict] | None:
    ...
    # Prefer a human-readable location description.
    # Do NOT prefer NALCO first: NALCO is a numeric accounting/location code,
    # which is why the website was showing values such as 224600 and 124300.
    name = (
        item.get("tps_description") or
        item.get("description") or
        item.get("stanox_location_description") or
        item.get("location_name") or
        item.get("crs_code") or
        item.get("crs") or
        item.get("nalco")
    )
...
def parse_schedule(record: dict, tiploc_lookup: dict[str, str]) -> tuple[dict, list[dict]] | None:
    ...
    service_row = {
        ...
        "origin_name": readable_location_name(tiploc_lookup, origin_tiploc),
        "origin_departure": clean_time(origin_loc.get("departure") or origin_loc.get("public_departure")),
        "destination_tiploc": dest_tiploc,
        "destination_name": readable_location_name(tiploc_lookup, dest_tiploc),
    ...
    for idx, loc in enumerate(locations):
        ...
        location_rows.append({
            ...
            "sort_order": idx,
            "tiploc": tiploc,
            "location_name": readable_location_name(tiploc_lookup, tiploc),
